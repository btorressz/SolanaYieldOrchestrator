...
class JupiterClient:
    ...
    def get_price(self, token_symbol: str, vs_currency: str = "USDC") -> Optional[float]:
        ...
        if quote:
            output_decimals = 6
            return quote.output_amount / (10 ** output_decimals)
        return None
...
class JupiterPriceAPI:
    ...
    def get_price(self, token_ids: List[str]) -> Dict[str, float]:
        try:
            ...
            for token_id, info in data.get("data", {}).items():
                prices[token_id] = info.get("price", 0.0)
            return prices
    ...
    def get_sol_usdc_price(self) -> float:
        ...
        return prices.get(SOL_MINT, 0.0)
        ...
        with self._lock:
            if not self._latencies:
                return {
                    "avg_latency_ms": 0,
                    "p95_latency_ms": 0,
                    "failure_rate": 0 if self._total == 0 else self._failures / self._total,
            ...
            return {
                ...
                "failure_rate": round(self._failures / self._total if self._total > 0 else 0, 4),
                "total_requests": self._total,
                "last_error": self._last_error,
                "last_error_timestamp": self._last_error_ts
            }
...
class EnhancedJupiterClient(JupiterClient):
    ...
    def _parse_route_info(self, quote: SwapQuote) -> RouteInfo:
        ...
        for plan_step in quote.route_plan:
            swap_info = plan_step.get("swapInfo", {})
            
            pool_type = swap_info.get("label", "unknown")
            route_labels.append(pool_type)
            
            fee_bps = float(swap_info.get("feeAmount", 0)) / max(1, float(swap_info.get("inAmount", 1))) * 10000
            total_fee_bps += fee_bps
            
            hop = RouteHop(
                pool_address=swap_info.get("ammKey", ""),
                pool_type=pool_type,
                input_mint=swap_info.get("inputMint", ""),
                output_mint=swap_info.get("outputMint", ""),
                fee_bps=fee_bps,
                liquidity=0.0,
                expected_output=int(swap_info.get("outAmount", 0))
            )
            hops.append(hop)
        ...
        mev_risk_level, safe_mode_allowed = self._assess_mev_risk(route_labels, hops, quote.price_impact_pct)
        ...
        return RouteInfo(
            ...
            estimated_slippage_bps=quote.slippage_bps,
            mev_risk_level=mev_risk_level,
            safe_mode_allowed=safe_mode_allowed
        )
    
    def _assess_mev_risk(self, route_labels: List[str], hops: List[RouteHop], price_impact_pct: float) -> tuple:
        """
        Assess MEV risk based on pool types and route characteristics.
        Returns (mev_risk_level, safe_mode_allowed)
        """
        ...
        elif has_high_mev or price_impact_pct > 1.0:
            mev_risk_level = "high"
            safe_mode_allowed = False
        elif has_medium_mev or price_impact_pct > 0.3 or len(hops) > 2:
        ...
        return mev_risk_level, safe_mode_allowed
    ...
    def compare_cross_venue_prices(
        self,
        prices: Dict[str, Dict[str, float]],
        sanity_threshold_bps: float = None
    ) -> Dict[str, Any]:
        threshold = sanity_threshold_bps or self.sanity_threshold_bps
        ...
        for symbol in ["SOL", "BTC", "ETH"]:
            ...
            if "jupiter" in prices and symbol in prices["jupiter"]:
                venue_prices["jupiter"] = prices["jupiter"][symbol]
            if "coingecko" in prices and symbol in prices["coingecko"]:
                venue_prices["coingecko"] = prices["coingecko"][symbol]
            if "kraken" in prices and symbol in prices["kraken"]:
                venue_prices["kraken"] = prices["kraken"][symbol]
            ...
            if mid_price > 0:
                max_deviation = max(abs(p - mid_price) / mid_price * 10000 for p in price_values)
            else:
                max_deviation = 0
        ...
        overall_safe = all(r.get("is_safe", True) for r in results.values())
    ...
    def get_route_depth_info(
        ...
        for plan_step in quote.route_plan:
            swap_info = plan_step.get("swapInfo", {})
            route_labels.append(swap_info.get("label", "unknown"))
        
        depth_estimate = self._estimate_depth_to_slippage(
            input_mint, output_mint, base_amount, target_slippage_bps=100
        )
        
        total_fee_bps = 0.0
        for plan_step in quote.route_plan:
            swap_info = plan_step.get("swapInfo", {})
            in_amt = float(swap_info.get("inAmount", 1))
            fee_amt = float(swap_info.get("feeAmount", 0))
            if in_amt > 0:
                total_fee_bps += (fee_amt / in_amt) * 10000
        ...
        return RouteDepthInfo(
            ...
            price=quote.price,
            slippage_bps=quote.price_impact_pct * 100,
            depth_to_1pct_slippage=depth_estimate,
    ...
    def compare_routes(
        ...
        for slippage in [25, 50, 100, 200]:
            ...
            if quote:
                route_labels = [
                    p.get("swapInfo", {}).get("label", "?")
                    for p in quote.route_plan
                ]
                ...
                routes.append({
                    ...
                    "output_amount": quote.output_amount,
                    "price_impact_pct": quote.price_impact_pct,
                    "price_impact_bps": quote.price_impact_pct * 100,
        ...
        if profile == "best_price":
            routes.sort(key=lambda r: -r["output_amount"])
        elif profile == "best_depth":
            routes.sort(key=lambda r: r["price_impact_bps"])
        elif profile == "most_stable":
            routes.sort(key=lambda r: (r["hops"], r["price_impact_bps"]))
    ...
    def get_slippage_curve(
        ...
        for size in sizes:
            ...
            if quote:
                slippage_bps = 0
                if base_price > 0:
                    slippage_bps = ((base_price - quote.price) / base_price) * 10000
                
                curve.append(SlippageCurvePoint(
                    size=float(size),
    ...
    def get_impact_analysis(
        ...
        prices = [p.effective_price for p in curve]
        
        max_size_100bps = max_size
        for point in curve:
            if point.slippage_bps >= 100:
                max_size_100bps = point.size
                break
    ...
    def get_price_impact_curve(
        ...
        DECIMALS_MAP = {
            "SOL": 9, "mSOL": 9, "MSOL": 9, "jitoSOL": 9, "JITOSOL": 9,
            "USDC": 6, "USDT": 6, "BTC": 8, "ETH": 8, "RAY": 6, "SRM": 6
        }
        ...
        for size in sizes:
            ...
            if quote:
                ...
                slippage_bps = 0
                if base_price > 0:
                    price_diff = abs(quote.price - base_price) / base_price
                    slippage_bps = price_diff * 10000
                
                fee_estimate = size * quote.price * 0.0005
    ...
    def _generate_mock_impact_curve(
        ...
        base_price = base_prices.get(base_symbol.upper(), 100.0)
        ...
        for i in range(points):
            ...
            slippage_bps = 2.0 + (size_factor ** 1.5) * 100 + (hash_val % 10)
            
            effective_price = base_price * (1 + slippage_bps / 10000) if side == "buy" else base_price * (1 - slippage_bps / 10000)
    ...
    def get_routes_comparison(
        ...
        for route in routes[:limit]:
            depth_estimate = size * (100 / max(route.get("price_impact_bps", 1), 0.1))
            ...
            enhanced_routes.append({
                ...
                "est_slippage_bps": route.get("price_impact_bps", 0),
                "est_depth_to_1pct": round(depth_estimate, 2),
                "total_fee_bps": round(route.get("price_impact_bps", 0) * 0.5 + 5, 2),
                "is_direct": route.get("is_direct", False)
            })
    ...
    def _generate_mock_routes(
        ...
        for i in range(min(limit, 5)):
            slippage = 5 + i * 3 + (hash_val % 10)
            price = base_price * (1 + (slippage / 10000) if side == "buy" else 1 - (slippage / 10000))
            ...
            routes.append({
                ...
                "est_depth_to_1pct": round(size * (100 / slippage), 2),
                "total_fee_bps": round(3 + i * 2, 2),
                "is_direct": i == 0
            })
