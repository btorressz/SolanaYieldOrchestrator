...
class PythClient:
    ...
    def __init__(self, enabled: bool = False, max_deviation_bps: int = 100):
    ...
    def _fetch_pyth_prices(self) -> Dict[str, PythPriceData]:
        ...
        try:
            ...
            response = self._session.get(self.PYTH_API_URL, params=params)
            ...
            for feed in data:
                feed_id = feed.get("id", "").replace("0x", "")
                ...
                if symbol:
                    ...
                    prices[symbol] = PythPriceData(
                        ...
                        status=feed.get("status", "trading")
                    )
    ...
    def _get_mock_prices(self) -> Dict[str, PythPriceData]:
        ...
        for symbol, base_price in base_prices.items():
            ...
            variation = (hash_val % 100 - 50) / 10000  # -0.5% to +0.5%
            price = base_price * (1 + variation)
            
            conf_ratio = 0.0005 + (hash_val % 10) / 20000  # 0.05% to 0.10%
            
            prices[symbol] = PythPriceData(
                symbol=symbol,
                price=price,
                confidence=price * conf_ratio,
                expo=-8 if symbol not in ["BONK"] else -12,
                publish_time=now,
                status="trading"
            )
    ...
    def get_oracle_confidence_bps(self, symbol: str, composite_price: float) -> Optional[int]:
        ...
        if pyth_data and pyth_data.price > 0:
            conf_ratio = pyth_data.confidence / pyth_data.price
            confidence_penalty = conf_ratio * 10000
        
        return int(deviation + confidence_penalty)
    ...
    def get_oracle_status(self, symbol: str, composite_price: float) -> str:
        ...
        if deviation <= self._deviation_thresholds["clean"]:
            return "clean"
        elif deviation <= self._deviation_thresholds["watch"]:
            return "watch"
        else:
            return "flagged"
    ...
    def is_oracle_trusted(self, symbol: str, composite_price: float) -> bool:
        ...
        return deviation <= self._max_deviation_bps
    ...
    def get_health_summary(self, composite_prices: Dict[str, float]) -> Dict[str, Any]:
        ...
        """
        symbols_data = []
        counts = {"clean": 0, "watch": 0, "flagged": 0, "unknown": 0}
        ...
        for symbol in self.PYTH_PRICE_IDS.keys():
            ...
            if composite_price and composite_price > 0:
                deviation = self.get_pyth_deviation(symbol, composite_price)
                status = self.get_oracle_status(symbol, composite_price)
            else:
                deviation = None
                status = "unknown"
...
def get_pyth_client() -> PythClient:
    ...
    if _pyth_client is None:
        ...
        _pyth_client = PythClient(
            enabled=getattr(Config, 'PYTH_ENABLED', False),
            max_deviation_bps=getattr(Config, 'MAX_ORACLE_DEVIATION_BPS', 100)
        )
    return _pyth_client
